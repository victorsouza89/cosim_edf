...
def simulate_custom_input(show_plot=True):
    ...
    # initialize
    controls_functions = [without_control, proportional_control]
    fmus = []
    unzipdirs = []
    for i in range(len(controls_functions)):
        #extract the FMU
        unzipdirs.append(extract(fmu_filename))
    ...
    passed_outputs = {i : [0.0] for i in range(len(controls_functions))} # list to record the outputs
    passed_inputs = {i : [0.0] for i in range(len(controls_functions))} # list to record the inputs
    ...
    for i in range(len(controls_functions)):
        list_types.append(('input - control '+str(i), np.float64))
        list_types.append(('output - control '+str(i), np.float64))
